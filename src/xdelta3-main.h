...
__attribute__((noinline, visibility ("default"))) int xdelta3_patch(int32_t s, int32_t i, int32_t o)
{
     main_file sfile;
     main_file ifile;
     main_file ofile;
     ...
     lseek(sfile.file, 0, SEEK_SET);
     lseek(ifile.file, 0, SEEK_SET);
     lseek(ofile.file, 0, SEEK_SET);

     reset_defaults();

     int res =  main_input (CMD_DECODE, & ifile, & ofile, & sfile);

     option_source_filename = "";
     program_name = "";
